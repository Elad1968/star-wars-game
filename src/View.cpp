...
View::View(size_t size, double zoom, double x, double y)
    : size(size)
    , zoom(zoom)
    , x(x)
    , y(y) {
...
void View::setSize(size_t s) {
    if (s <= sizeBounds.first || sizeBounds.second <= s)
        throw std::invalid_argument("View setSize must be between " + std::to_string(sizeBounds.first) + " and " +
                                    std::to_string(sizeBounds.second) + ".");
    size = s;
    makeXAxis();
    makeYAxis();
}
...
void View::show() {
    std::cout << "Display setSize: " << size << ", scale: " << zoom << ", origin: (" << x / Model::scale << ", "
              << y / Model::scale << ")" << std::endl;
    makeMatrix();
