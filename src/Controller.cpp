...
Controller::Controller()
    ...
    creatorCommand = {
        {"shuttle",
         [&model](const std::vector<std::string> &args) -> void {
        ...
        {"destroyer",
         [&model](const std::vector<std::string> &args) -> void {
             if (args.size() != 6)
                 throw std::invalid_argument("Usage: create destroyer <name> <agent_name> (<x>, <y>)");
    ...
    modelViewCommands = {
        ...
        {"go",
         [this, &model](const std::vector<std::string> &args) -> void {
         ...
         [&model](const std::vector<std::string> &args) -> void {
             if (args.size() != 4)
                 throw std::invalid_argument(
                     "Usage: <shuttle_name> transport <space_station_name> <fortress_star_name>");
    ...
    while (true) {
        try {
            ...
            if (modelViewCommands.find(args[0]) != modelViewCommands.end()) {
                ...
                if (spaceship->status() == Spaceship::DEAD)
                    throw std::runtime_error(spaceship->getName() + " is dead and cannot operate.");
                Commands::mapped_type function;
                try {
