...
void Model::transport(const std::string &spaceshipName,
...
const std::shared_ptr<Spaceship> &Model::findSpaceship(const std::string &name) const {
    auto iterator = std::find_if(
        spaceships.begin(), spaceships.end(), [&name](const std::shared_ptr<Spaceship> &spaceship) -> bool {
...
void Model::explode(const Destroyer::Rocket &rocket) {
    for (const std::shared_ptr<Spaceship> &spaceship : spaceships) {
        if (spaceship->getLocation() == rocket.getLocation() &&
            std::dynamic_pointer_cast<Falcon>(spaceship) != nullptr) {
            spaceship->die();
        }
    }
    auto it =
        std::remove_if(rockets.begin(), rockets.end(), [&rocket](const decltype(rockets)::value_type &rocket2) -> bool {
...
const std::shared_ptr<Agent> &Model::findAgent(const std::string &name) const {
    auto iterator =
        std::find_if(agents.begin(), agents.end(), [&name](const decltype(agents)::value_type &pair) -> bool {
            if (pair->getName() == name) return true;
            return false;
        });
    if (iterator != agents.end()) return *iterator;
    auto it = std::find_if(
        spaceships.begin(), spaceships.end(), [&name](const decltype(spaceships)::value_type &spaceship) -> bool {
...
bool Model::isBomberNearby(const Object::Point &point) {
    return std::any_of(
        spaceships.begin(), spaceships.end(), [&point](const decltype(spaceships)::value_type &spaceship) -> bool {
